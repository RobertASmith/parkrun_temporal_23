...
# create a GLM model
# first model fits to the pre-lockdown period only
# ignores seasonality
# uses orthoganal polynomial order 3 because there is non-linear growth
...
mod0a <- glm(data = dt_timeseries,
            formula = finishers ~ poly(t, 3) * covid_period)
# third one includes weekly seasonality...
# every week is included as a factor - multiplicatively
mod1 <- glm(data = dt_timeseries,
            formula = finishers ~ poly(t, 3) * covid_period * week)
...
#==================================#
# Next steps:
# Improve model, probably poisson and make specific to region?
# Incorporate number of runs as predictor?
