...
dt_fweeks <- data.table::data.table(t = 0:n_weeks,
                        four_week = as.factor(rep(
                          1:13, each = 4, times = n_weeks / (13 * 4)
                        )))
#left-join with df_timeseries to account for
#the five-rows for each week (one for each IMD quintile)
df_timeseries <- dplyr::left_join(x = df_timeseries,
                           y = dt_fweeks,
                           by = "t")
...
df_timeseries_total <- df_timeseries |>
  dplyr::group_by(date, covid, post_covid, four_week) |>
  dplyr::summarise(
...
reg_table <-
  dplyr::bind_rows(
    coef_table(fit_glm(df_timeseries, "Most deprived 20%"), imd = "Most deprived 20%"),
...
reg_table <- reg_table |>
  dplyr::mutate(dplyr::across(.cols = c(estimate, lower_bound, upper_bound),  ~
                                round(., 2)))
#combine OR and 95% CI
reg_table$rate_ratio <- stringr::str_c(reg_table$estimate, " (", reg_table$lower_bound,"-",reg_table$upper_bound,")", sep = "" )
reg_table <- reg_table[,c("term", "imd_q5", "rate_ratio")]
#pivot wider for report
reg_table <- tidyr::pivot_wider(data = reg_table , names_from = term, values_from = rate_ratio )
...
preds <-
  dplyr::bind_rows(
    pred_func(df_timeseries, "Most deprived 20%"),
...
res <- data.table::data.table(t = 0:n_weeks,
...
# see R folder for custom function.
preds_ex <-
  dplyr::bind_rows(
    fit_glm_ex(df_timeseries, "Most deprived 20%"),
...
four_week_coef <- generics::tidy(mod_total, exp =T) |>
  dplyr::filter(grepl("four_week", term)) |>
  #number in accordance to the 'four-week' variable in df_timeseries
  dplyr::mutate(four_week = c(2:13)) |>
  dplyr::select(four_week, estimate)
...
pred_smooth <- dplyr::left_join(pred_smooth, four_week_coef, by = "four_week")
...
#merge the datasets
df_lost <- merge(x = preds_pc,
                 y = obs_pc,
                 by = c("date", "imd_q5"))


#keep desired columns
df_lost <- data.table::data.table(
  date = df_lost$date,
...
total_lost <- df_lost |>
  ...
  dplyr::summarise(predicted = sum(predicted),
            ...
            lost = sum(lost)) |>
  dplyr::mutate(total_lost = sum(lost),
         perc_decrease = signif(lost *100/predicted,3),
...
reopen_lost <- df_lost |>
  dplyr::filter(covid_period == "post-lockdown") |>
  dplyr::group_by(imd_q5) |>
  dplyr::summarise(
  ...
  dplyr::mutate(total_reopen_lost = sum(lost),
                perc_decrease = signif(lost * 100 / predicted, 3),
                predicted = round(predicted, 0),
                observed = round(observed, 0),
                lost = round(lost, 0)
...
#combine into table
lost_table <- dplyr::left_join(x = reopen_lost,
                               y = total_lost,
                               by = "imd_q5")
#output table
write.csv(lost_table,
          "outputs/lost_table.csv")
...
df_timeseries_extrap <- data.table::data.table(date = seq(from = as.Date("2015-01-03"),
...
# Custom functions, see R folder.
mod_total <- fit_glm_total(df_timeseries_total)
mod_total_ex <- fit_glm_ex_total(df_timeseries_total)

pred_extrap_obs <- data.table::data.table(date = df_timeseries_extrap$date,
                              finishers = predict(mod_total, df_timeseries_extrap, type = "response"),
                              post_covid = df_timeseries_extrap$post_covid)

pred_extrap_exp <- data.table::data.table(date = df_timeseries_extrap$date,
